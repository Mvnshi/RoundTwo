...
// real value on the next commit, so both sides agree.
  ...
  return (
    <motion.button
      ...
      /*
        Local adaptation: upstream dropped `whileTap` entirely under reduced
        motion. Motion auto-assigns `tabIndex` to any element carrying a tap
        gesture, so removing the prop removed the attribute — and since the
        preference only resolves on the client, the server emitted
        `tabindex="0"` and the client did not. Keeping the prop and flattening
        its value holds the rendered shape steady.
      */
      whileTap={disabled ? undefined : { scale: reduce ? 1 : 0.98 }}
      transition={SPRING_PRESS}
      className={cn(
        // Local adaptation: square, and on this site's tokens.
        "relative inline-grid h-16 w-full min-w-0 touch-none place-items-center overflow-hidden border border-ink bg-ink px-8 text-paper sm:w-auto sm:min-w-72",
        "[--hold-radius:22px]",
        TOUCH_GESTURE_CLASS,
        "outline-none focus-visible:ring-2 focus-visible:ring-hazard-ink focus-visible:ring-offset-2 focus-visible:ring-offset-paper",
    ...
    >
      ...
      >
        <motion.span
          ...
          className={cn(
            "absolute inset-0 bg-hazard will-change-[opacity,transform]",
            fillClassName,
          )}
        >
          {!reduce ? (
            type === "horizontal" ? (
              <motion.svg
                ...
                className="absolute -right-5 top-0 h-[200%] w-6 text-hazard"
              >
                <path
              ...
              <motion.svg
                ...
                className="absolute -top-5 left-0 h-6 w-[200%] text-hazard"
              >
                <path
