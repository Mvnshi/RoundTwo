...
// Local adaptation: motion's own `useReducedMotion` resolves only on the
// client, so branching *rendered output* on it mismatches the server HTML.
// This one returns false during SSR and hydration, then re-renders.
...
/*
  Local adaptation: upstream ships emerald/amber semantics that clash with this
  site's cream, chartreuse and coral palette. Mapped onto our tokens instead —
  success is the brand accent because "recovered" is the one thing it marks.
*/
const STATUS_CLASS: Record<AnimatedBadgeStatus, string> = {
  ...
  success: "border-transparent bg-brand text-brand-foreground",
  warning: "border-lost-soft/40 bg-lost-soft/15 text-lost",
  danger: "border-lost/25 bg-lost-soft/15 text-lost",
  loading: "border-transparent bg-foreground text-background",
};
